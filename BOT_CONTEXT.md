...
- Keyboard viewport regression fix `1.7.14`: when the iOS keyboard is open and focus is inside an editable field, `ChatLayout` must still apply `visualViewport.offsetTop` compensation to keep the chat pinned, but must not call `window.scrollTo(0, 0)` on those editable-driven viewport scroll events. Skipping the whole handler causes the entire chat + composer to drift with the keyboard; forcing window scroll causes the old jitter.
- Batch `1.8.0`: new direct chats navigate immediately into the created conversation; New Chat modal respects safe-area bounds better; direct/group deletion is available from the chat avatar modal and from a small avatar menu in the conversation list; message image lightbox supports swipe-down close; conversations support pinned messages via context menu with a pinned bar in chat; DM invite sending is race-safe on the server via a conversation-scoped advisory lock; mobile safe-area bottom padding moved into real surfaces (composer/footer/invitation bar) instead of leaving a stray layout strip.
- Hotfix `1.8.1`: when creating a direct chat, blur and close the modal before navigating so the gray overlay does not linger; deleting a chat from inside ChatPane must always navigate directly to `/` instead of relying on history; encrypted inline images open in the same fullscreen Lightbox as regular images instead of forcing download on tap.
